...
impl HtpasswdDatabase {
    pub fn from_file(htpasswd_file_path: &Path) -> Result<HtpasswdDatabase, Error> {
        ...
        let file = File::open(htpasswd_file_path)
            .map_err(|io_error| Error::CannotOpenHtpasswdFile { path_string: path_string.clone(), io_error })?;
        let reader = BufReader::new(file);
        let mut registered_users = HashMap::new();
        for (i, line_res) in reader.lines().enumerate() {
            let owned_line = line_res
                .map_err(|io_error| Error::CannotReadHtpasswdFile { path_string: path_string.clone(), io_error })?;
            ...
            if parts.len() != 2 {
                return Err(Error::MalformedHtpasswdLine { path_string: path_string.clone(), line: i });
            }
            let user = parts[0];
            let base64_sha1_password = parts[1];
            let sha1_password = base64::decode(base64_sha1_password)
                .map_err(|_| Error::InvalidPasswordString { path_string: path_string.clone(), line: i })?;
            if registered_users.contains_key(user) {
                return Err(Error::DuplicateUser { path_string: path_string.clone(), line: i, user: user.to_owned() })
            }
